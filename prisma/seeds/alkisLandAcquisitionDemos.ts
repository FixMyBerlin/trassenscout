...
/**
 * One seed project per Bundesland to exercise `AcquisitionAreaMap` + ALKIS WFS proxy.
 * Centers come from `src/data/alkisStateTestCoordinates.ts` (STATE_TEST_COORDINATES).
 */
...
>[] {
  return ALKIS_LAND_ACQUISITION_DEMO_ENTRIES.map((d) => ({
    slug: d.slug,
    subTitle: d.subTitle,
    description: `Autogenerated seed project for ALKIS-WFS / AcquisitionAreaMap (${d.alkisStateKey}).`,
    logoSrc: null,
    partnerLogoSrcs: [],
    exportEnabled: false,
    aiEnabled: false,
...
export function buildAlkisLandAcquisitionDemoSubsection(
  ...
  const lineSub: Omit<Prisma.SubsubsectionUncheckedCreateInput, "subsectionId"> = {
    ...
    costEstimate: null,
    qualityLevelId: null,
    description: "Seed LINE inside subsection polygon for AcquisitionAreaMap / WFS bbox tests.",
    mapillaryKey: null,
    managerId: null,
    maxSpeed: null,
    trafficLoad: null,
