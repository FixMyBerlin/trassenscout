...
/** Dev: repo-local folder for inspection. Staging/prod: OS temp (deploy roots are often read-only). */
function alkisWfsTempBaseDir() {
  return isDev ? join(process.cwd(), "tmp", "alkis-wfs") : tmpdir()
}
...
export async function convertWfsResponseToGeoJson(
  ...
  const tempBaseDir = alkisWfsTempBaseDir()
  ...
  const tempPath = join(tempBaseDir, name)
