...
import { model } from "@/src/components/core/ai/models"
import {
  createProjectContextPromptSection,
  type CreateProjectContextPromptSectionParams,
} from "@/src/server/ai/createProjectContextPromptSection"
import { runWithLangfuseAiObservation } from "@/src/server/observability/langfuseObservation.server"

type SummarizePdfUploadParams = {
  pdfData: Buffer
  userId: string | number
  projectContext: CreateProjectContextPromptSectionParams["projectContext"]
}

export async function summarizePdfUpload({
  pdfData,
  userId,
  projectContext,
}: SummarizePdfUploadParams) {
...
`
      ...
      const { text } = await generateText({
        ...
        messages: [
          ...
          {
            role: "user",
            content: [{ type: "file", data: pdfData, mediaType: "application/pdf" }],
          },
        ],
