...
} from "@/src/server/admin/admin.functions"

export const AdminAlkisLandAcquisitionDemoTools = () => {
  const seedMutation = useMutation({ mutationFn: seedAlkisLandAcquisitionDemosFn })
  const deleteMutation = useMutation({ mutationFn: deleteAlkisLandAcquisitionDemosFn })
  const [message, setMessage] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)

  const busy = seedMutation.isPending || deleteMutation.isPending

  const handleSeed = async () => {
    setError(null)
    setMessage(null)
    try {
      const result = await seedMutation.mutateAsync({ data: {} })
  ...
  return (
    <section className="my-8 rounded-sm border border-amber-200 bg-amber-50 p-5">
      ...
      <p className="mt-2 max-w-2xl text-sm text-gray-700">
        ...
        <code className="rounded bg-white px-1">VITE_APP_ENV</code> auf{" "}
