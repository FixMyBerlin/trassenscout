...
const isDrawModeEnabled = (
  ...
  switch (mode) {
    case "point":
      return showPoint && enabledButtons.point
    case "linestring":
      return showLine && enabledButtons.linestring
    case "freehand-linestring":
      return showLine && enabledButtons["freehand-linestring"]
    case "polygon":
      return showPolygon && enabledButtons.polygon
  }
}
...
function getFirstEnabledDrawMode(
  ...
  for (const mode of DRAW_MODES_ORDER) {
    if (isDrawModeEnabled(mode, enabledButtons, showPoint, showLine, showPolygon)) return mode
  }
  return "linestring"
}
