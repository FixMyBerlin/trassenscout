...
type Props = {
  ...
  onGeometryTypeChange?: (geometryType: string | null) => void
...
export const useTerraDrawControl = ({
  ...
  const control = useControl<TerraDrawControl>(
    () => {
      let terraDrawControl: TerraDrawControl

      const wrappedOnChange = (geometry: SupportedGeometry | null, geometryType: string | null) => {
        setEnabledButtons(terraDrawControl.getEnabledButtons())
        onGeometryTypeChange?.(geometryType)
        onChange?.(geometry, geometryType)
      }

      terraDrawControl = new TerraDrawControl(wrappedOnChange, initialGeometry, modeConfig)
      onGeometryTypeChange?.(initialGeometry?.type ?? null)
      terraDrawControl.setOnModeChange((nextMode) => {
        setModeState(nextMode)
        onModeChange?.(nextMode)
      })
      terraDrawControl.setOnButtonsChange(() => {
        setEnabledButtons(terraDrawControl.getEnabledButtons())
      })
      terraDrawControl.setOnSelectionChange(() => {
        setSelectedIds(terraDrawControl.getSelectedIds())
      })
      return terraDrawControl
    },
    { position: "top-left" },
  )
