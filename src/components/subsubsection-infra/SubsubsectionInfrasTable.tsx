...
type Props = {
  subsubsectionInfras: any[]
}

export const SubsubsectionInfrasTable = ({ subsubsectionInfras }: Props) => {
  const { projectSlug } = loggedInProjectRouteApi.useParams()
  const { search, editLink } = useSubsubsectionInfraRouteLinks(projectSlug)
  const { deleteRow } = useSubsubsectionInfraMutations(projectSlug, search)
  ...
  const handleDelete = async (subsubsectionInfraId: number) => {
    if (window.confirm(`Den Eintrag mit ID ${subsubsectionInfraId} unwiderruflich löschen?`)) {
      try {
        await deleteRow(subsubsectionInfraId)
      } catch {
        alert(
