...
import type { UploadProtocolEntry } from "./useUploadProtocol"
...
const summaryLine = (entries: UploadProtocolEntry[]) => {
  ...
  const matched = entries.filter((entry) => entry.slugAssignment?.kind === "matched").length
  const unmatched = entries.filter((entry) => entry.slugAssignment?.kind === "unmatched").length
...
export const UploadProtocolReport = ({ entries, finished, onDismiss }: Props) => {
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {finished ? summaryLine(entries) : "Upload läuft…"}
          </p>
        </div>
        <button
          type="button"
      ...
      <ul className="-mx-2 mt-3 divide-y divide-gray-100">
        {entries.map((entry, index) => {
          ...
          const isUnmatched = entry.slugAssignment?.kind === "unmatched"
          return (
            <li
            ...
            >
              ...
              {entry.slugAssignment?.kind === "matched" && (
                <span className="text-xs text-gray-500">
                  ...
                  <Link
                    ...
                    params={{
                      projectSlug,
                      subsectionSlug: entry.slugAssignment.subsectionSlug,
                      subsubsectionSlug: entry.slugAssignment.subsubsectionSlug,
                    }}
                    className="text-blue-600 underline hover:text-blue-800"
                  >
                    {entry.slugAssignment.subsectionSlug}/{entry.slugAssignment.subsubsectionSlug}
                  </Link>
                </span>
              )}
              {isUnmatched && (
