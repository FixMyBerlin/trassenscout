...
import { UploadDropzoneView } from "./UploadDropzoneView"

type UploadDropzoneDescription = {
  fileTypes?: string
  maxFileSize?: string
  maxFiles?: number
}
...
type Props = {
  ...
  /** Runs on the full file selection before any byte moves. Return null to cancel the batch. */
  prepareUpload?: (
    files: File[],
  ) => PreparedUploadBatch | null | Promise<PreparedUploadBatch | null>
  ...
  accept?: string
  description?: UploadDropzoneDescription | string
}
...
export const UploadDropzone = ({
  ...
  const prepareBatch = async (files: File[]) => {
    try {
      const prepared = prepareUpload ? await prepareUpload(files) : { files }
      if (!prepared) return null
      return { ...prepared, metadata: { ...viewerUploadMeta, ...prepared.metadata } }
    } catch (error) {
      console.error("Error preparing upload batch:", error)
      const errorString =
        error instanceof Error ? error.message : "Ein unbekannter Fehler ist aufgetreten."
      setUploadError(translateServerError(errorString))
      return null
    }
  }
