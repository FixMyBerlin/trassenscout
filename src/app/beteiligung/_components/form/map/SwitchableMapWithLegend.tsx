...
import { SwitchableMap } from "@/src/app/beteiligung/_components/form/map/SwitchableMap"
...
type Props = {
  label: string
  description?: string
  mapProps: ComponentProps<typeof SwitchableMap>
  legendProps: ComponentProps<typeof SurveyMapLegend>
}

export const SwitchableMapWithLegend = ({ mapProps, legendProps, label, description }: Props) => {
  ...
  return (
    <MapProvider>
      <div className={clsx("mt-8 mb-12 w-full p-2", hasError && "rounded-sm bg-red-50")}>
        ...
        <SwitchableMap description={description} {...mapProps} />
