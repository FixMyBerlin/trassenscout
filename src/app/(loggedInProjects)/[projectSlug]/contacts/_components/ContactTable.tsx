...
export const ContactTable = ({ contacts, currentUserEmail, projectSlug }: Props) => {
  ...
  const handleSubmit = async (values: z.infer<typeof contactMailSchema>) => {
    const selectedContactIds = values.selectedContacts.map(Number)
  ...
  return (
    <Form
      className="mt-7"
      schema={contactMailSchema}
      initialValues={{ selectedContacts: [] }}
      onSubmit={handleSubmit}
      onValuesChange={(values) => {
        setMailButtonActive(values.selectedContacts.length > 0)
      }}
        ...
        <>
          ...
          <ButtonWrapper className="mt-6 justify-between">
            ...
            <form.Subscribe selector={(s) => s.isSubmitting}>
              {(isSubmitting) => (
                <button
                  disabled={!mailButtonActive || isSubmitting}
