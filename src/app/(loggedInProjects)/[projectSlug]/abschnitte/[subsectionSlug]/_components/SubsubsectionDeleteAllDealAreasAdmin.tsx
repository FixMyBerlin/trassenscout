...
import deleteAllDealAreasForSubsubsection from "@/src/server/dealAreas/mutations/deleteAllDealAreasForSubsubsection"
...
export const SubsubsectionDeleteAllDealAreasAdmin = ({
  projectSlug,
  subsectionSlug,
  subsubsectionSlug,
  subsubsectionId,
}: Props) => {
  const [deleteMutation, { isLoading }] = useMutation(deleteAllDealAreasForSubsubsection)
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      invalidateQuery(getDealAreasBySubsubsection)
      invalidateQuery(getProjectRecordsBySubsubsection)
      invalidateQuery(getUploadsWithSubsections)
      setFeedback(
        `Gelöscht: ${result.deletedDealAreas} Dealfläche(n). Hinweis: verknüpfte Projektdokumentation zu diesen Flächen kann entfallen (Datenbank-Cascade).`,
      )
    } catch (e: unknown) {
