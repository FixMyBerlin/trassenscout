...
import { uploadEmailAttachment } from "@/src/server/ProjectRecordEmails/uploadEmailAttachment"
...
export async function POST(request: Request) {
  try {
    ...
    const uploadPromises = attachments.map(async (attachment) => {
      ...
      try {
        // Upload to S3
        const uploadedFile = await uploadEmailAttachment(attachment, projectRecordEmail.projectId)
    ...
    subsections = await db.subsection.findMany({
      where: { projectId: projectRecordEmail.projectId },
      select: {
        id: true,
        slug: true,
        start: true,
    ...
    console.log(
      `Found ${projectRecordTopics.length} projectRecord topics for project ${projectRecordEmail.projectId}`,
    )
...
`,
    ...
    // Create a DB entry ProjectRecord with AI-extracted data
    ...
    console.log("Created ProjectRecord:", projectRecord)
    ...
    return Response.json({
      ...
      projectRecordId: projectRecord.id,
      uploadIds: uploadIds,
      message: `ProjectRecord created successfully with ${createdUploads.length} attachment(s)`,
    })
  } catch (error) {
