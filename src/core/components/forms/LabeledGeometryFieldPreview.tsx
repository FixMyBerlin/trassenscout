...
export const LabeledGeometryFieldPreview = ({ form, name, hasError }: Props) => {
  return (
    <form.Subscribe
      selector={(s) => ({
        geometry: s.values[name] as SupportedGeometry | undefined,
        geometryType: (s.values.type ?? GeometryTypeEnum.LINE) as GeometryTypeEnum,
      })}
    >
          ...
          >
            ...
            {schemaResult.success ? (
              ...
              <p className="mt-2 mb-0 min-h-[300px] text-sm">
                <strong>Achtung Validierung:</strong> Dieser Fehler muss behoben werden. Aus
                technischen Gründen kann man das Formular trotzdem speichern. Das würde dann aber zu
                einer defekten Applikation führen.
              </p>
            )}
          </div>
        )
