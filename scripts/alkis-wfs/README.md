...
- `bun scripts/alkis-wfs/audit.ts` (`bun run alkis-audit`)
...
1. Run audit. `bun run alkis-audit`
2. Review markdown report and open TODO/unverified states.
3. Run config update. `bun run alkis-update-config`
...
- Probe coordinates live in `src/data/alkisStateTestCoordinates.ts` (`STATE_TEST_COORDINATES`); they are representative and can still hit empty areas.
